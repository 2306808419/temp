...
QEMUOPTS = -machine virt -bios $(OPENSBI_DIR)/build/platform/generic/firmware/fw_jump.bin 
QEMUOPTS += -kernel $K/kernel -m 128M -smp $(CPUS) -nographic
QEMUOPTS += -global virtio-mmio.force-legacy=false
# QEMUOPTS += -drive file=fs.img,if=none,format=raw,id=x0
# QEMUOPTS += -device virtio-blk-device,drive=x0,bus=virtio-mmio-bus.0

qemu: $K/kernel $(OPENSBI_DIR)/build/platform/generic/firmware/fw_jump.bin
	$(QEMU) $(QEMUOPTS)
